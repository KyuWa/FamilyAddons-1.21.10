...
object PearlCalculator {
    ...
    private val acceleration = Vec3d(0.0, -0.03, 0.0)
     ...
     */
    private fun velocityGivenTime(time: Double, displacement: Vec3d): Vec3d {
        // (displacement * DRAG) - (acceleration * time)
        val numer = displacement.multiply(DRAG).subtract(acceleration.multiply(time))
        // 1 - exp(-DRAG * time)  ==  -expm1(-DRAG * time)
        val divisor = -expm1(-DRAG * time)
        // Avoid divide-by-zero for time→0
        val first = if (abs(divisor) < 1e-12) numer else numer.multiply(1.0 / divisor)
        // acceleration / DRAG = acceleration * 100 (terminal velocity vector)
        val second = acceleration.multiply(1.0 / DRAG)
    ...
    private fun findLookDirAndTime(
        ...
        val f: (Double) -> Double = { t ->
            ...
            abs(v.lengthSquared() - squaredSpeed)
        }
    ...
    fun solvePearl(
        ...
        val aimPoint = spawnPos.add(lookDir.multiply(aimDist))
