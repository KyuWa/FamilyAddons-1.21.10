 ...
 *
 * Implementation:
 *   1. Key polling here — on press: capture player.yaw + 180 and player.pitch
 *      into freelookYaw/Pitch, force THIRD_PERSON_BACK. On release/toggle-off:
 *      restore previous perspective.
 *   2. Mouse delta capture — FreelookEntityMixin intercepts the local player's
 ...
 *
 * Mouse sensitivity is multiplied by 0.15 to match vanilla's internal cursor-
...
object CameraHelper {
    ...
    private fun tickFreelook(client: MinecraftClient) {
        val cfg = FamilyConfigManager.config.utilities
    ...
    private fun enableFreelook(client: MinecraftClient) {
        ...
        freelookYaw = player.yaw + 180f
        freelookPitch = player.pitch.coerceIn(-90f, 90f)
        freelookActive = true
    ...
    private fun disableFreelook(client: MinecraftClient) {
        freelookActive = false
        savedPerspective?.let { client.options.perspective = it }
    ...
    private fun tickNoFrontCamera(client: MinecraftClient) {
