 ...
 * independently, and avoids two scans of `world.entities` per tick.
 *
...
object KuudraGiants {
    ...
    private val _giants = LinkedHashSet<GiantEntity>()
    private val _zombies = LinkedHashSet<ZombieEntity>()

    /** Currently-tracked giants. Read-only view; safe to iterate. */
    val giants: Set<GiantEntity> get() = _giants

    /** Currently-tracked zombies. Used by KuudraCrateWaypoints for hitbox rendering. */
    val zombies: Set<ZombieEntity> get() = _zombies

    /**
     * Compute crate world position from a giant's position + yaw.
     * Same formula as PawsUp's published values.
     */
    fun cratePosFor(g: GiantEntity): Vec3d {
        val angleRad = Math.toRadians(g.yaw + CRATE_YAW_BIAS_DEG)
        return Vec3d(
    ...
    fun getCratePositions(): List<Vec3d> = _giants.map { cratePosFor(it) }
    ...
    private fun shouldScan(): Boolean {
        ...
        // Whitelist-gated feature — only scan if user is whitelisted AND has it on.
        if (org.kyowa.familyaddons.Whitelist.isAllowed() && cfg.hidden.crateWaypointsEnabled) return true
        return false
    }
    ...
    private fun scan() {
        ...
        for (entity in world.entities) {
            ...
            when (entity) {
                is GiantEntity  -> _giants.add(entity)
                is ZombieEntity -> _zombies.add(entity)
                else -> {}
            }
        }
    }
